...
A power pellet lasts 9 seconds on every board (Stronger makes that pellet 4 seconds instead). A thick open ring sits on the ghost house, with a clear center so the house stays visible. It starts full and drains to empty over that time; it has no digits. Eating any ghost while less than 1.5 seconds remain adds 1.5 seconds, and the ring refills to match the new remaining time.
