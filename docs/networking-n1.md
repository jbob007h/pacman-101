...
- A local maze death sends `deathReport`. The server confirms with `playerEliminated`. Pressure at 100 eliminates a CPU seat only. A human is eliminated by that death report (a non-frightened board ghost or a red jammer) or by disconnecting during play, and `matchEnd` names the winner. The client then uses the existing win congratulations or death standings.
